...
async def get_anime_manga(
    ...
    if search_type == "anime_anime":
        ...
        if anime_malid != "Unknown":
            anime_vars = {"idMal": anime_malid, "asHtml": True, "type": "ANIME"}
        elif _id:
            anime_vars = {"id": _id, "asHtml": True, "type": "ANIME"}
        else:
            anime_vars = {"search": search_str, "asHtml": True, "type": "ANIME"}
        anime_result = await anime_json_synomsis(anime_query, anime_vars)
...
async def get_anime_name(name, return_id: bool = False):
    for anime in (await searchanilist(name))[0]:
        ...
        return anime_name if not return_id else (anime_name, anime["id"])
...
def get_pmanga(name=None, id=None):
    ...
    caption = textwrap.dedent(
        ...
        <b>→Status:</b> <code>{manga["release_status"].title()}</code>
