...
from bot.utils.functions import async_wrap, post_to_telegraph
...
async def get_anime_manga(
    ...
    if search_type == "anime_anime":
        variables = {"id": _id} if _id else {"search": search_str}
        query = anime_query
        result = await anime_json_synomsis(query, variables)
        res = list(result.keys())
        if "errors" in res:
            return (
                f"<b>Error</b> : <code>{result['errors'][0]['message']}</code>",
                None,
                None,
            )
    ...
    if search_type == "anime_anime":
        ...
        if html_char:
            ...
            html_pc += "<em>(Spoiler Territory!)</em><br>"
            html_pc += html_char
            html_pc += "<br><br>"
        ...
        html_pc += f"\n<a href='{anilist_animelink}'>View on anilist.co</a>"
    ...
    return (
        (caption, image, reply_markup, result)
        if re_json
        else (caption, image, reply_markup)
    )
...
async def get_anime_name(name, return_id: bool = False):
    for anime in (await searchanilist(name))[0]:
        title = anime["title"]
        anime_name = title.get("english") or title.get("romaji")
        return (anime_name, anime["id"]) if return_id else anime_name
...
def make_pmanga_text(
    title, rating, status, chapters, genres: list, link=None, tags: list = None
):
    text = textwrap.dedent(
        ...
        <b>→Genres:</b> <code>{", ".join(natsorted(genres))}</code>
        """
    )
...
async def get_pmanga(name=None, id=None, return_manga=False):
    if not (name or id):
        raise ValueError("Either give the name to search for or the manga id.")
    variables = {"search": name} if name else {"id": id}
    manga = (await anime_json_synomsis(manga_query, variables))["data"]["Media"]
    if manga is None:
        return None
    if return_manga:
        return manga
    ...
    return caption, manga["coverImage"]["extraLarge"]
