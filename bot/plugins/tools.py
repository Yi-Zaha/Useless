...
@Client.on_message(filters.command("logs") & filters.user(ALLOWED_USERS))
...
@Client.on_message(filters.command("noformat") & filters.user(ALLOWED_USERS))
...
@Client.on_message(filters.command("ls") & filters.user(SUDOS))
    ...
    for i in omk:
        try:
            ...
            if os.path.isdir(name):
                ...
                if hb(size):
                    result_text += f"{emoji} `{nam}`  `{hb(size)}" + "`\n"
                    fos += size
                else:
                    result_text += f"{emoji} `{nam}`" + "\n"
                foc += 1
            else:
