...
@Client.on_message(filters.command("getmanga") & filters.user(ALLOWED_USERS))
...
@Client.on_callback_query(filters.regex(r"^manganl_(.*)"))
...
<b>[Synopsis]({description})</b>"""
    ...
    buttons = [
        InlineKeyboardButton(
            "Download",
            f"mpage:{btn_user}:{manga_id}:0" if btn_user else f"mpage:{manga_id}:0",
        )
    ]
...
@Client.on_message(filters.command("mread") & filters.user(ALLOWED_USERS))
...
@Client.on_message(filters.command("mbulk") & filters.user(ALLOWED_USERS))
async def bulk_manga(client, message):
    ...
    rid = get_random_id()
    id = f"cancel_bulk:{message.from_user.id}:{rid}"
    BULK_PROCESS.add(id)
        ...
        try:
            ...
            if not merge_limit:
                ch_msg = await client.send_document(
                    ...
                    file,
                    caption=(
                        f"<b>Password:</b> <code>{file_pass}</code>"
...
@Client.on_callback_query(filters.regex(r"^cancel_bulk:.*"))
...
@Client.on_callback_query(filters.regex("^mpage:(.*)$"))
...
@Client.on_callback_query(filters.regex("^mgdl:(.*)"))
