...
SUBS_CHANNEL = Config.get("SUBS_CHANNEL", -1001817093909)
...
@Client.on_message(filters.private & filters.incoming & ~filters.user(SUDOS))
...
@Client.on_message(filters.reply & filters.private & filters.user(OWNER_ID))
async def reply_to_pms(client, message):
    ...
    try:
        ...
        await client.send_message(
            LOG_CHAT,
            f"<b>Someone has Blocked Me❗</b>\n\n<b>›› Name →</b> <code>{user.first_name + (user.last_name or '')}</code>\n<b>›› Username →</b> <code>{f'@{user.username}' if user.username else 'N/A'}</code>\n<b>›› Profile Link →</b> [Click Here](tg://user?id={user.id})",
        )
...
@Client.on_message(filters.command("storefiles") & filters.user(SUDOS))
